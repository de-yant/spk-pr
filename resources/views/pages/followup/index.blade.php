            ...
            <div class="p-5 sm:p-8">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3 w-full lg:w-auto">
                        ...
                        <form method="GET" action="{{ route('follow-up.index') }}"
                        ...
                        {{-- LIMIT (KIRIM per_page, PERTAHANKAN q) --}}
                        <form method="GET" action="{{ route('follow-up.index') }}"
            ...
            <div class="p-4 sm:p-6">
                ...
                <div class="mt-4 overflow-x-auto -mx-4 sm:mx-0">
                        ...
                        <table class="w-full text-sm">
                            <thead class="text-left">
                                <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A]">
                                    ...
                                    <th class="py-3 pr-4 font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Tanggal
                                        Followup</th>
                                    <th class="py-3 pr-4 font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Respon
                                        Followup</th>
                            ...
                            <tbody>
                                @forelse($items as $it)
                                    <tr class="border-b border-[#f0f0ee] dark:border-[#2a2a28]">
                                        ...
                                        <td class="py-3 pr-4 text-[#1b1b18] dark:text-[#EDEDEC]">
                                            {{ $it->calonKonsumen->nama ?? '-' }}
                                            <div class="text-xs text-[#706f6c] dark:text-[#A1A09A]">
                                                {{ $it->calonKonsumen->no_hp ?? '' }}
                                            </div>
                                        </td>

                                        <td class="py-3 pr-4 text-[#1b1b18] dark:text-[#EDEDEC]">
                                            {{ \Carbon\Carbon::parse($it->tgl_followup)->format('d/m/Y') }}
                                        </td>
                                        <td class="py-3 pr-4">
                                            @php
                                                $badge = match ($it->respon_followup) {
                                                    1
                                        ...
                                        <td class="py-3 pr-2 text-right align-middle" x-data="{ open: false }">
                                            <div class="flex items-center justify-end gap-2 flex-wrap">
                                                <a href="{{ route('follow-up.show', $it->id) }}"
                   ...
                   bg-blue-600 hover:bg-blue-700 text-white transition shadow-sm">
                                                ...
                                                <a href="{{ route('follow-up.edit', $it->id) }}"
                                                    class="inline-flex items-center gap-1 px-3 h-9 rounded-md text-sm
                   ...
                   shadow-[0px_18px_45px_rgba(0,0,0,0.30)] overflow-hidden"
                               ...
                               hover:border-[#1915014a] dark:hover:border-[#62605b] transition">
                                                            ...
                                                            <form action="{{ route('follow-up.destroy', $it->id) }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit"
                                    ...
                                    <tr>
                                        <td colspan="5"
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                @else
                                                    <div>Belum ada data follow up.</div>
                                                    <a href="{{ route('follow-up.create') }}"
                                                        class="inline-flex items-center justify-center px-5 py-2 rounded-sm
